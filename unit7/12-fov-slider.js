...
function init() {
    ...
    eyeTargetScale =
        Math.tan((camera.fov * (Math.PI / 180)) / 2) * startdir.length();
}
    ...
    for (x = -4500; x <= 4500; x += 1000) {
        for (z = -4500; z <= 4500; z += 1000) {
            ...
            var block = new THREE.Mesh(
                new THREE.CubeGeometry(100, 300, 100),
                blockMaterial
            );
...
function createSupport(bsupport) {
    ...
    // base
    cube = new THREE.Mesh(
        new THREE.BeveledBlockGeometry(
            20 + 64 + 110,
            4,
            2 * 77 + 12,
            bevelRadius
    ...
    cube = new THREE.Mesh(
        new THREE.BeveledBlockGeometry(64, 104, 6, bevelRadius),
        footMaterial
    );
    ...
    // legs
    cube = new THREE.Mesh(
        new THREE.BeveledBlockGeometry(60, 282 + 4, 4, bevelRadius),
        legMaterial
    );
    cube.position.x = 0; // centered on origin along X
...
function createBody(bbody) {
    ...
    cylinder = new THREE.Mesh(
        new THREE.CylinderGeometry(104 / 2, 104 / 2, 0, 32),
        bodyMaterial
    );
    ...
    cylinder = new THREE.Mesh(
        new THREE.CylinderGeometry(12 / 2, 12 / 2, 390 - 100, 32),
        bodyMaterial
    );
    cylinder.position.x = 0;
